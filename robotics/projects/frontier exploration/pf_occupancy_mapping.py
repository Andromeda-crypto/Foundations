...
from robotics.localization.ekf_localization import Robot
from robotics.localization.particle_filter_localization import Particle, ParticleFilter
from robotics.mapping.ocuupancy_grid_mapping import OccupancyGridMap
from robotics.planning.astar_planner import AStarPlanner
from robotics.mapping.costmap import CostMap
from robotics.planning.frontier_detector import FrontierDetector
from robotics.experiments.exploration_metrics import ExplorationMetrics
from robotics.mapping.visited_map import VisitedMap
from robotics.localization.uncertainty import ParticleUncertainty
from robotics.experiments.experiment_logger import ExperimentLogger
from robotics.planning.frontier_memory import FrontierMemory
...
stuck_steps = 0
previous_distance_to_goal = None
max_stuck_steps = 30
...
for step in range(num_steps):
    ...
    if goal_world is not None and planned_path is not None:
        if previous_distance_to_goal is not None:
            progress = previous_distance_to_goal - distance_to_goal
    ...
    if step >= mapping_steps and step % replan_interval == 0:
        ...
        print(f"Frontier Clusters: {len(frontier_clusters)}")
        print(f"Failed Goals: {len(frontier_memory.failed_goals)}")
        ...
        if goal_world is None:
            ...
            if goal_world is None:
                ...
                if scan_failures >= max_scan_failures:
                    ...
                    print(f"Frontier Clusters: {len(frontier_clusters)}")
                    print(f"Failed Goals: {len(frontier_memory.failed_goals)}")
                    print(f"Mapped Percent: {current_metrics['mapped_percent']:.2f}")
        ...
        if goal_world is not None:
            ...
            print(f"Goal before planning: {goal_world}")
            ...
            if new_path is not None:
                ...
                print("Planner failed to reach selected frontier.")
                print("Failed goal:", goal_world)
    ...
    if len(failed_frontier_goals) > 0:
        fx = [p[0] for p in failed_frontier_goals]
        fy = [p[1] for p in failed_frontier_goals]
        plt.scatter(
            fx,
            fy,
            color='red',
