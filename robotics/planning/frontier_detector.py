...
class FrontierDetector:
    ...
    def choose_frontier_goal(
        ...
        for cluster in clusters:
            ...
            if costmap is not None and costmap.is_blocked(center_gx, center_gy):
                best_cell = None
                best_cell_distance = float("inf")

                for cell_gx, cell_gy in cluster:
                    if costmap.is_blocked(cell_gx, cell_gy):
                        continue
                    ...
                    if cell_distance < best_cell_distance:
                        best_cell_distance = cell_distance
                        best_cell = (cell_gx, cell_gy)

                if best_cell is None:
                    continue

                center_gx, center_gy = best_cell
                candidate_goal = self.map.grid_to_world(center_gx, center_gy)
        ...
        if best_cluster is None or best_goal is None:
            return None
